function stitchTiffs_greenChan(data_dir)
tic;
...
for n=1:numel(stacks)
    ...
    if mod(n,batchLen)==1    %for every x trials, start a new .tiff file
        if (n+batchLen-1) <= numel(stacks)  %if end of stack, use the final trial as savefile-name
            savFile_name{j}=[FileName(1:end-3) '_trials_' int2str(n) '-' int2str(n+batchLen-1) ext];
        else
            savFile_name{j}=[FileName(1:end-3) '_trials_' int2str(n) '-' int2str(numel(stacks)) ext]; 
        end
        ...
        cd(data_dir);
        cd(image_subdir);
        curr_frame = imread(img_fileID, 'Index', num_chans, 'Info', info);
   
        t.setTag(tagstruct);
        t.write(curr_frame); %write first frame
        for k = 2*num_chans:num_chans:stack_frames(n) %add on subsequent frames; stitches on every (num_chan)-th frame 
       ...
       for k = num_chans:num_chans:stack_frames(n) %add on subsequent frames; stitches on every (num_chan)-th frame 
            curr_frame = imread(img_fileID, 'Index', k, 'Info', info);    
            t.writeDirectory();
            t.setTag(tagstruct);
            t.write(curr_frame);
